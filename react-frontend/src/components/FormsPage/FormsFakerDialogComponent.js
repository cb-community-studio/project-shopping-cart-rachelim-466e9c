...
import formsFakerFactory from "./formsFakerFactory";
...
const FormsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = formsFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("forms").create(elem));
...
export default connect(null, mapDispatch)(FormsFakerDialogComponent);
